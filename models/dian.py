...
tributes = {
    ...
    '08': 'ReteCREE', '20': 'FtoHorticultura', '21': 'Timbre', '22': 'Bolsas', '23': 'INCarbono',
    '24': 'INCombustibles',
...
class DianDocument(models.Model):
    ...
    document_type = fields.Selection([('f', 'Factura'), ('c', 'Nota/Credito'), ('d', 'Nota/Debito')],
                                     string="Tipo de documento", readonly=True)
    resend = fields.Boolean(string="Autorizar reenvio?", default=False)
    email_response = fields.Selection([('accepted', 'ACEPTADA'), ('rejected', 'RECHAZADA'), ('pending', 'PENDIENTE')],
                                      string='Decisión del cliente', required=True, default='pending', readonly=True)
    ...
    xml_response_contingency_dian = fields.Text(string='Mensaje de respuesta DIAN al envío de la contigencia',
                                                readonly=True)
    ...
    def _get_resolution_dian(self, data_header_doc):
        ...
        rec_active_resolution = data_header_doc.journal_id.sequence_id.dian_resolution_ids.filtered(
            lambda r: r.resolution_number == data_header_doc.resolution_number)
        ...
        if rec_active_resolution:
            ...
            dict_resolution_dian[
                'InvoiceAuthorization'] = rec_active_resolution.resolution_number  # Número de resolución
            ...
            dict_resolution_dian[
                'TechnicalKey'] = rec_active_resolution.technical_key  # Clave técnica de la resolución de rango
    ...
    def request_validating_dian(self, document_id):
        ...
        if response_dict['s:Envelope']['s:Body']['GetStatusZipResponse']['GetStatusZipResult']['b:DianResponse'][
            ...
            if dian_document.contingency_4 == False:
                if self.enviar_email(dian_document.xml_document, dian_document.document_id.id,
                                     dian_document.xml_file_name):
        ...
        if dic_result_verify_status['result_verify_status'] == False:
            ...
            if document_type == 'c':
                ...
                for by_validate_cn in by_validate_credit_notes:
                    invoice_validated = self.env['account.invoice'].search(
                        [('move_name', '=', by_validate_cn.document_id.origin), ('type', '=', 'out_invoice'),
                         ('state_dian_document', '=', 'exitoso')])
                    if invoice_validated:
                        cn_with_validated_invoices_ids.append(by_validate_cn.id)
                    else:
                        raise ValidationError(
                            'La factura a la que se le va a aplicar la nota de crédito, no ha sido enviada o aceptada por la DIAN')
            ...
            if document_type == 'd':
                ...
                for by_validate_cn in by_validate_debit_notes:
                    ...
                    if invoice_validated:
                        ...
                        raise ValidationError(
                            'La factura a la que se le va a aplicar la nota de débito, no ha sido enviada o aceptada por la DIAN')
            ...
            for doc_send_dian in docs_send_dian:
                ...
                if data_constants_document['InvoiceTypeCode'] in ('01', '04', '02'):
                    ...
                    data_taxs_xml = self._generate_taxs_data_xml(template_tax_data_xml, data_taxs,
                                                                 data_constants_document['CurrencyID'], document_type)
                    ...
                    CUFE = self._generate_cufe(data_header_doc.id, data_constants_document['InvoiceID'],
                    ...
                    data_xml_document = self._generate_data_fe_document_xml(template_basic_data_fe_xml, dian_constants,
                                                                            data_constants_document, data_taxs_xml,
                                                                            data_lines_xml, CUFE, data_xml_signature)
                ...
                if data_constants_document['InvoiceTypeCode'] == '91':
                    ...
                    data_credit_lines_xml = self._generate_credit_lines_data_xml(template_credit_line_data_xml,
                                                                                 data_header_doc.id,
                                                                                 data_constants_document['CurrencyID'])
                    # Generar CUDE
                    ambiente = tipo_ambiente['PRODUCCION'] if company.production else tipo_ambiente['PRUEBA']
                    CUFE = self._generate_cude(data_header_doc.id, data_constants_document['InvoiceID'],
                                               data_constants_document['IssueDateCufe'],
                                               data_constants_document['IssueTime'],
                                               data_constants_document['LineExtensionAmount'],
                                               dian_constants['SupplierID'],
                                               data_constants_document['CustomerSchemeID'],
                                               data_constants_document['CustomerID'], dian_constants['PINSoftware'],
                                               data_constants_document['PayableAmount'],
                                               data_taxs, ambiente)
                    doc_send_dian.cufe = CUFE
                    ...
                    data_xml_document = self._generate_data_nc_document_xml(template_basic_data_nc_xml, dian_constants,
                                                                            data_constants_document,
                                                                            data_credit_lines_xml, CUFE, data_taxs_xml)
                ...
                if data_constants_document['InvoiceTypeCode'] == '92':
                    ...
                    data_taxs_xml = self._generate_taxs_data_xml(template_tax_data_xml, data_taxs,
                                                                 data_constants_document['CurrencyID'], document_type)
                    # Detalle líneas de nota de crédito
                    data_debit_lines_xml = self._generate_debit_lines_data_xml(template_debit_line_data_xml,
                                                                               data_header_doc.id,
                                                                               data_constants_document['CurrencyID'])
                    # Generar CUFE
                    ambiente = tipo_ambiente['PRODUCCION'] if company.production else tipo_ambiente['PRUEBA']
                    ...
                    data_xml_document = self._generate_data_nd_document_xml(template_basic_data_nd_xml, dian_constants,
                                                                            data_constants_document,
                                                                            data_debit_lines_xml, CUFE, data_taxs_xml)
                    # Elimina espacios del documento xml
                    data_xml_document = etree.tostring(etree.XML(data_xml_document, parser=parser))
                    data_xml_document = data_xml_document.decode()
                ...
                if data_constants_document['InvoiceTypeCode'] == '03':
                    ...
                    data_xml_document = self._generate_data_contingencia_document_xml(
                        template_basic_data_contingencia_xml, dian_constants, data_constants_document, data_taxs_xml,
                        data_lines_xml, CUFE, data_xml_signature)
                    # Elimina espacios del documento xml la factura
                    data_xml_document = etree.tostring(etree.XML(data_xml_document, parser=parser))
                    data_xml_document = data_xml_document.decode()
                ...
                if company.production:
                    ...
                    data_xml_send = self._generate_SendTestSetAsync_send_xml(template_SendTestSetAsyncsend_xml,
                                                                             fileName,
                                                                             Document, Created, testSetId,
                ...
                #   Contruye XML de envío de petición
                headers = {'content-type': 'application/soap+xml'}
                URL_WEBService_DIAN = server_url['PRODUCCION_VP'] if company.production else server_url[
                    'HABILITACION_VP']
                ...
                if company.in_contingency_4 == False:  # Diferente a contingencia tipo 4 (Problemas tecnológicos en la DIAN)
                    ...
                    if response.status_code != 200:  # Respuesta de envío no exitosa
                        ...
                        if company.production:
                            ...
                            if dict_result_verify_status['result_verify_status'] == True:
                                return
                            dict_mensaje = \
                            response_dict['s:Envelope']['s:Body']['SendBillSyncResponse']['SendBillSyncResult'][
                                'b:IsValid']
                            ...
                            if response_dict['s:Envelope']['s:Body']['SendBillSyncResponse']['SendBillSyncResult'][
                                'b:StatusCode'] == '00':
                                doc_send_dian.response_message_dian = \
                                response_dict['s:Envelope']['s:Body']['SendBillSyncResponse']['SendBillSyncResult'][
                                    'b:StatusCode'] + ' '
                                ...
                                data_header_doc.write({'diancode_id': doc_send_dian.id})
                                # Generar código QR
                                doc_send_dian.QR_code = self.sudo()._generate_barcode(dian_constants,
                                                                                      data_constants_document, CUFE,
                                                                                      data_taxs)
                                # Envío de correo
                            ...
                            if '@i:nil' in dict_mensaje:
                                if response_dict['s:Envelope']['s:Body']['SendTestSetAsyncResponse'][
                                    'SendTestSetAsyncResult']['b:ErrorMessageList']['@i:nil'] == 'true':
                    ...
                    doc_send_dian.xml_send_query_dian = data_xml_send
                    # Generar código QR
                    doc_send_dian.QR_code = self.sudo()._generate_barcode(dian_constants, data_constants_document, CUFE,
                                                                          data_taxs)
    ...
    def _get_dian_constants(self, data_header_doc):
        ...
        dian_constants = {}
        dian_constants[
            'document_repository'] = company.document_repository  # Ruta en donde se almacenaran los archivos que utiliza y genera la Facturación Electrónica
        dian_constants[
            'Username'] = company.software_identification_code  # Identificador del software en estado en pruebas o activo
        dian_constants['Password'] = hashlib.new('sha256',
                                                 company.password_environment.encode()).hexdigest()  # Es el resultado de aplicar la función de resumen SHA-256 sobre la contraseña del software en estado en pruebas o activo
        dian_constants['IdentificationCode'] = partner.country_id.code  # Identificador de pais
        dian_constants[
            'ProviderID'] = partner.xidentification if partner.xidentification else ''  # ID Proveedor de software o cliente si es software propio
        ...
        dian_constants['SoftwareSecurityCode'] = self._generate_software_security_code(
            ...
            data_header_doc.number)  # Código de seguridad del software: (hashlib.new('sha384', str(self.company_id.software_id) + str(self.company_id.software_pin)))
        ...
        dian_constants['ProfileExecutionID'] = tipo_ambiente['PRODUCCION'] if company.production else tipo_ambiente[
            'PRUEBA']  # 1 = produccción 2 = prueba
        dian_constants[
            'SupplierAdditionalAccountID'] = '1' if partner.is_company else '2'  # Persona natural o jurídica (persona natural, jurídica, gran contribuyente, otros)
        dian_constants[
            'SupplierID'] = partner.xidentification if partner.xidentification else ''  # Identificador fiscal: En Colombia, el NIT
        ...
        dian_constants[
            'SupplierCountrySubentity'] = partner.state_id.name  # Ciudad o departamento tabla 6.4.2 res.country.state
        dian_constants['SupplierCountrySubentityCode'] = partner.xcity.code[
                                                         0:2]  # Ciudad o departamento tabla 6.4.2 res.country.state
        ...
        dian_constants[
            'SupplierRegistrationName'] = company.trade_name  # Razón Social: Obligatorio en caso de ser una persona jurídica. Razón social de la empresa
        ...
        dian_constants['SupplierTaxLevelCode'] = self._get_partner_fiscal_responsability_code(
            partner.id)  # tabla 6.2.4 Régimes fiscal (listname) y 6.2.7 Responsabilidades fiscales
        ...
        dian_constants['CertDigestDigestValue'] = self._generate_CertDigestDigestValue(company.digital_certificate,
                                                                                       dian_constants['CertificateKey'],
                                                                                       dian_constants[
                                                                                           'document_repository'],
                                                                                       dian_constants[
        ...
        dian_constants['DeliveryAddress'] = partner.partner_shipping_id.street if hasattr(partner,
                                                                                          'hasattr') else partner.street
    ...
    def _generate_data_constants_document(self, data_header_doc, dian_constants, document_type, in_contingency_4):
        ...
        # Generar nombre del archvio xml
        data_constants_document['FileNameXML'] = self._generate_xml_filename(data_resolution, NitSinDV,
                                                                             data_header_doc.type,
                                                                             data_header_doc.is_debit_note)
        data_constants_document['FileNameZIP'] = self._generate_zip_filename(data_resolution, NitSinDV,
                                                                             data_header_doc.type,
                                                                             data_header_doc.is_debit_note)
        data_constants_document['InvoiceAuthorization'] = data_resolution[
            'InvoiceAuthorization']  # Número de resolución
        ...
        data_constants_document['ContingencyID'] = data_resolution[
            'ContingencyID'] if document_type == 'contingency' else ' '  # Número de documento dian
        data_constants_document['Nonce'] = self._generate_nonce(data_resolution['InvoiceID'], dian_constants[
            'SeedCode'])  # semilla para generar números aleatorios
        data_constants_document['TechnicalKey'] = data_resolution[
            'TechnicalKey']  # Clave técnica de la resolución de rango
        data_constants_document['LineExtensionAmount'] = self._complements_second_decimal_total(
            data_header_doc.amount_untaxed)  # Total Importe bruto antes de impuestos: Total importe bruto, suma de los importes brutos de las líneas de la factura.
        ...
        data_constants_document['TotalTaxInclusiveAmount'] = self._complements_second_decimal_total(
        ...
        data_constants_document['TaxExclusiveAmount'] = self._complements_second_decimal_total(data_constants_document[
                                                                                                   'TaxExclusiveAmount'])  # Total Base Imponible (Importe Bruto+Cargos-Descuentos): Base imponible para el cálculo de los impuestos
        # Valor Bruto más tributos - Valor del Descuento Total + Valor del Cargo Total - Valor del Anticipo Total
        data_constants_document['PayableAmount'] = self._complements_second_decimal_total(
            data_header_doc.amount_without_wh_tax)  # Total de Factura: Total importe bruto + Total Impuestos-Total Impuesto Retenidos
        ...
        data_constants_document['IssueDate'] = date_invoice_cufe[
            'IssueDate']  # Fecha de emisión de la factura a efectos fiscales
        ...
        data_constants_document['InvoiceTypeCode'] = self._get_doctype(data_header_doc.type,
                                                                       data_header_doc.is_debit_note, in_contingency_4)
        data_constants_document['CreditNoteTypeCode'] = self._get_doctype(data_header_doc.type,
                                                                          data_header_doc.is_debit_note,
                                                                          in_contingency_4)
        data_constants_document['DebitNoteTypeCode'] = self._get_doctype(data_header_doc.type,
                                                                         data_header_doc.is_debit_note,
                                                                         in_contingency_4)  # Tipo de Factura, código: facturas de venta, y transcripciones; tipo = 1 para factura de venta
        ...
        if data_header_doc.partner_id.doctype == 31:
            data_constants_document[
                'IDAdquiriente'] = data_header_doc.partner_id.xidentification if data_header_doc.partner_id.xidentification else ''
        ...
        data_constants_document[
            'CustomerID'] = data_header_doc.partner_id.xidentification if data_header_doc.partner_id.xidentification else ''  # Identificador fiscal: En Colombia, el NIT
        ...
        data_constants_document['CustomerPartyName'] = self._replace_character_especial(
            data_header_doc.partner_id.name)  # Nombre Comercial
        data_constants_document[
            'CustomerDepartment'] = data_header_doc.partner_id.state_id.name if data_header_doc.partner_id.state_id.name else ''
        data_constants_document[
            'CustomerCityCode'] = data_header_doc.partner_id.xcity.code  # Municipio tabla 6.4.3 res.country.state.city
        data_constants_document[
            'CustomerCityName'] = data_header_doc.partner_id.xcity.name  # Municipio tabla 6.4.3 res.country.state.city
        data_constants_document[
            'CustomerCountrySubentity'] = data_header_doc.partner_id.state_id.name  # Ciudad o departamento tabla 6.4.2 res.country.state
        data_constants_document['CustomerCountrySubentityCode'] = data_header_doc.partner_id.xcity.code[
                                                                  0:2]  # Ciudad o departamento tabla 6.4.2 res.country.state
        data_constants_document[
            'CustomerCountryCode'] = data_header_doc.partner_id.country_id.code  # País tabla 6.4.1 res.country
        data_constants_document[
            'CustomerCountryName'] = data_header_doc.partner_id.country_id.name  # País tabla 6.4.1 res.country
        data_constants_document['CustomerAddressLine'] = data_header_doc.partner_id.street
        data_constants_document['CustomerTaxLevelCode'] = self._get_partner_fiscal_responsability_code(
            data_header_doc.partner_id.id)
        data_constants_document['CustomerRegistrationName'] = self._replace_character_especial(
            data_header_doc.partner_id.companyName)
        data_constants_document[
            'CustomerEmail'] = data_header_doc.partner_id.email if data_header_doc.partner_id.email else ''
        ...
        if dian_constants['Currency'] == data_header_doc.currency_id.id:
            ...
            data_constants_document['CalculationRate'] = self._get_rate_date(data_header_doc.company_id.id,
                                                                             data_header_doc.currency_id.id,
                                                                             data_header_doc.date_invoice)
            data_constants_document['CalculationRate'] = self._complements_second_decimal_total(
        ...
        if data_constants_document['InvoiceTypeCode'] in ('91', '92'):
            ...
            invoice_cancel = self.env['account.invoice'].search(
                [('move_name', '=', data_header_doc.origin), ('type', '=', 'out_invoice'),
                 ('state_dian_document', '=', 'exitoso')])
            if invoice_cancel:
                dian_document_cancel = self.env['dian.document'].search(
                    [('state', '=', 'exitoso'), ('document_type', '=', 'f'),
...
"""
    ...
    def _generate_data_fe_document_xml(self, template_basic_data_fe_xml, dc, dcd, data_taxs_xml, data_lines_xml, CUFE,
                                       data_xml_signature):
        template_basic_data_fe_xml = template_basic_data_fe_xml % {'InvoiceAuthorization': dcd['InvoiceAuthorization'],
                                                                   ...
                                                                   'SupplierAdditionalAccountID': dc[
                                                                   ...
                                                                   'SupplierCountrySubentity': dc[
                                                                       'SupplierCountrySubentity'],
                                                                   'SupplierCountrySubentityCode': dc[
                                                                   ...
                                                                   'SupplierElectronicMail': dc[
                                                                       'SupplierElectronicMail'],
                                                                   'CustomerAdditionalAccountID': dcd[
                                                                   ...
                                                                   'CustomerCountrySubentity': dcd[
                                                                       'CustomerCountrySubentity'],
                                                                   'CustomerCountrySubentityCode': dcd[
                                                                   ...
                                                                   'CustomerElectronicMail': dcd[
                                                                       'CustomerElectronicMail'],
                                                                   ...
                                                                   'TotalLineExtensionAmount': dcd[
                                                                       'LineExtensionAmount'],
                                                                   'TotalTaxExclusiveAmount': dcd['TaxExclusiveAmount'],
                                                                   'TotalTaxInclusiveAmount': dcd[
                                                                   ...
                                                                   'SchemeNameAdquiriente': dcd[
                                                                       'SchemeNameAdquiriente'],
                                                                   'IDAdquiriente': dcd['IDAdquiriente'],
                                                                   'SupplierCityNameSubentity': dc[
                                                                       'SupplierCityNameSubentity'],
...
"""
    ...
    def _generate_data_contingencia_document_xml(self, template_basic_data_contingencia_xml, dc, dcd, data_taxs_xml,
                                                 data_lines_xml, CUFE, data_xml_signature):
...
<CreditNote xmlns="urn:oasis:names:specification:ubl:schema:xsd:CreditNote-2" xmlns:cac="urn:oasis:names:specification:ubl:schema:xsd:CommonAggregateComponents-2" xmlns:cbc="urn:oasis:names:specification:ubl:schema:xsd:CommonBasicComponents-2" xmlns:ds="http://www.w3.org/2000/09/xmldsig#" xmlns:ext="urn:oasis:names:specification:ubl:schema:xsd:CommonExtensionComponents-2" xmlns:sts="http://www.dian.gov.co/contratos/facturaelectronica/v1/Structures" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="urn:oasis:names:specification:ubl:schema:xsd:CreditNote-2    http://docs.oasis-open.org/ubl/os-UBL-2.1/xsd/maindoc/UBL-CreditNote-2.1.xsd">
    ...
    def _generate_data_nc_document_xml(self, template_basic_data_nc_xml, dc, dcd, data_credit_lines_xml, CUFE,
                                       data_taxs_xml):
...
<DebitNote xmlns="urn:oasis:names:specification:ubl:schema:xsd:DebitNote-2" xmlns:cac="urn:oasis:names:specification:ubl:schema:xsd:CommonAggregateComponents-2" xmlns:cbc="urn:oasis:names:specification:ubl:schema:xsd:CommonBasicComponents-2" xmlns:ds="http://www.w3.org/2000/09/xmldsig#" xmlns:ext="urn:oasis:names:specification:ubl:schema:xsd:CommonExtensionComponents-2" xmlns:sts="http://www.dian.gov.co/contratos/facturaelectronica/v1/Structures" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:schemaLocation="urn:oasis:names:specification:ubl:schema:xsd:DebitNote-2    http://docs.oasis-open.org/ubl/os-UBL-2.1/xsd/maindoc/UBL-DebitNote-2.1.xsd">
   <ext:UBLExtensions>
    ...
    def _generate_data_nd_document_xml(self, template_basic_data_nc_xml, dc, dcd, data_debit_lines_xml, CUFE,
                                       data_taxs_xml):
...
<rep:Document>%(Document)s</rep:Document>
    ...
    def _get_taxs_data(self, invoice_id):
        ...
        if data_tax_detail_doc:
            for item_tax in data_tax_detail_doc:
                ...
                tax_percentage_iva_01 = self.env['account.tax'].search([('id', '=',
                                                                         item_tax.tax_id.id)]).amount if item_tax.tax_id.tax_group_fe == 'iva_fe' else tax_percentage_iva_01
                tax_percentage_ica_03 = self.env['account.tax'].search([('id', '=',
                                                                         item_tax.tax_id.id)]).amount if item_tax.tax_id.tax_group_fe == 'ica_fe' else tax_percentage_ica_03
                tax_percentage_inc_04 = self.env['account.tax'].search([('id', '=',
                                                                         item_tax.tax_id.id)]).amount if item_tax.tax_id.tax_group_fe == 'ico_fe' else tax_percentage_inc_04  # Mod impuesto
                tax_percentage_ret_06 = self.env['account.tax'].search([('id', '=',
                                                                         item_tax.tax_id.id)]).amount if item_tax.tax_id.tax_group_fe == 'ret_fe' else tax_percentage_ret_06

                invoice_lines = self.env['account.invoice.line'].search(
                    [('invoice_id', '=', invoice_id), ('invoice_line_tax_ids', 'in', item_tax.tax_id.id)])
                ...
                if iva_01 and item_tax.tax_id.tributes == '01':
                    dic_taxs_data['iva_' + str(iva_lines)] = self._complements_second_decimal_total(
                        item_tax.amount_total)
                    dic_taxs_data['tax_percentage_iva_' + str(iva_lines)] = self._complements_second_decimal_total(
                        item_tax.tax_id.amount)
                    dic_taxs_data['total_base_iva_' + str(iva_lines)] = self._complements_second_decimal_total(
                        item_tax.base)
                    iva_lines += 1

                if ret_06:
                    dic_taxs_data['ret_' + str(ret_lines)] = self._complements_second_decimal_total(ret_06 * -1)
                    dic_taxs_data['tax_percentage_ret_' + str(ret_lines)] = self._complements_second_decimal_total(
                        tax_percentage_ret_06 * -1)
                    dic_taxs_data['total_base_ret_' + str(ret_lines)] = self._complements_second_decimal_total(
                        total_base_ret_06)
                    ret_lines += 1
        ...
        dic_taxs_data['tax_percentage_ica_03'] = self._complements_second_decimal_total(
            tax_percentage_ica_03 if tax_percentage_ica_03 >= 0.00 else tax_percentage_ica_03 * -1.00)
        dic_taxs_data['total_base_ica_03'] = self._complements_second_decimal_total(
            total_base_ica_03 if total_base_ica_03 >= 0.00 else total_base_ica_03 * -1.00)

        dic_taxs_data['inc_04'] = self._complements_second_decimal_total(inc_04)  # Mod impuesto
        dic_taxs_data['tax_percentage_inc_04'] = self._complements_second_decimal_total(
            tax_percentage_inc_04)  # Mod impuesto
        ...
        for data_line in data_lines_doc:
            ...
            if data_line.price_subtotal:
                ...
                ILInvoicedQuantity = self._complements_second_decimal_total(
                    data_line.quantity)  # 13.1.1.9 - Cantidad: Cantidad del artículo solicitado. Número de unidades servidas/prestadas.
                ILLineExtensionAmount = self._complements_second_decimal_total(
                    data_line.price_subtotal)  # 13.1.1.12 - Costo Total: Coste Total. Resultado: Unidad de Medida x Precio Unidad.
                ...
                ILAmount = self._complements_second_decimal_total(
                    data_line.discount)  # Valor Descuento: Importe total a descontar.
                ...
                if data_line.product_id.product_tmpl_id.operation_type == '11':
                    ...
                    if not data_line.invoice_id.mandante_id:
                        raise ValidationError(
                            _(u'El Propietario o Mandante no se encuentra seleccionado, en la pestaña otra información puede seleccionarlo'))
    ...
    def _generate_barcode(self, dian_constants, data_constants_document, CUFE, data_taxs):
        ...
        datos_qr = ' NumFac: ' + NumFac + ' FecFac: ' + FecFac + ' HorFac: ' + Time + ' NitFac: ' + NitOFE + ' DocAdq: ' + DocAdq + ' ValFac: ' + str(
            ValFac) + ' ValIva: ' + str(ValIva) + ' ValOtroIm: ' + str(ValOtroIm) + ' ValTotFac: ' + str(
            ValTotFac) + ' CUFE: ' + CUFE
        # Genera código QR
...
"""
    ...
    def _cron_validate_accept_email_invoice_dian(self):
        date_current = self._get_datetime()
        date_current = datetime.strptime(date_current, '%Y-%m-%d %H:%M:%S')
        rec_dian_documents = self.env['dian.document'].sudo().search(
            [('state', '=', 'exitoso'), ('email_response', '=', 'pending')])
    ...
    def generate_attachment_document(self, invoice, dian_document):
        ...
        data_constants_document = self._generate_data_constants_document(invoice, dian_constants, invoice.type,
                                                                         invoice.company_id.in_contingency_4)
